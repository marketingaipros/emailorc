...
test.describe("EmailORC manual QA coverage", () => {
  ...
  test("CSV upload, field mapping expectations, validation, generation, and AI role output", async ({ page }) => {
    ...
    await expect(page.getByText("3 records loaded · 7 columns detected")).toBeVisible();
    await expect(page.getByText(/map fields/i)).toBeVisible();
    await expect(page.getByText(/preview first 5 rows/i)).toBeVisible();
    await expect(page.getByText(/offer library item/i)).toBeVisible();
    await expect(page.locator("select").filter({ has: page.locator("option", { hasText: "Email" }) }).first()).toBeVisible();
    await page.getByRole("button", { name: /validate & generate drafts/i }).click();
    ...
    await expect(page.getByText("ORC validation", { exact: true })).toBeVisible();
    await expect(page.getByText("SENTINEL strategy", { exact: true })).toBeVisible();
    await expect(page.getByText("SCRIBE draft", { exact: true })).toBeVisible();
    await expect(page.getByText("LEXI QA", { exact: true })).toBeVisible();
